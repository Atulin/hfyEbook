...
import {Contents} from "../types/spec.js";
...
export function removeLast($: Root, coll: Cheerio[], selector: string, count: number) {
	const elems = $(selector);
...
export function removeMatching($: Root, coll: Cheerio[], selector: string, regexp: RegExp) {
	$(selector).each((i, e) => {
		const el = $(e);
		const t = el.text();
...
export function pruneParagraphs(chap: Contents, coll: Cheerio[], params: {[key: string]: [number, number, string[]]|[number, number]}) {
	const $ = chap.dom;
